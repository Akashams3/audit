...
import { Clock, Check, X, AlertTriangle, FileText, ArrowRight, ShieldCheck, Calendar, Search } from 'lucide-react';
...
const LateUploadRequestsPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const reqRes = await authFetch(`http://localhost:8080/api/director/late-upload-requests${buildYearParams()}`);
      if (reqRes.ok) {
      ...
      if (user?.role === 'ROLE_DIRECTOR') {
        const filesRes = await authFetch(`http://localhost:8080/api/director/files${buildYearParams()}`);
        if (filesRes.ok) {
          filesList = await filesRes.json();
        }
      } else if (user?.role === 'ROLE_INVIGILATOR') {
        const cRes = await authFetch(`http://localhost:8080/api/invigilator/academic-files${buildYearParams()}`);
        const dRes = await authFetch(`http://localhost:8080/api/invigilator/department-files${buildYearParams()}`);
        let cList = [];
        let dList = [];
  ...
  const requestsSearchHook = useFilterSortSearch(requests, {
    searchKeys: ['faculty.name', 'faculty.department.code', 'schedule.title', 'reason', 'status']
  });

  const lateFilesSearchHook = useFilterSortSearch(lateFiles, {
    searchKeys: ['faculty.name', 'department', 'fileName', 'documentType', 'courseName']
  });
        ...
        <>
          <div className="flex justify-end mb-4">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder={activeTab === 'requests' ? "Search requests..." : "Search late files..."}
                value={activeTab === 'requests' ? requestsSearchHook.searchTerm : lateFilesSearchHook.searchTerm}
                onChange={(e) => {
                  if (activeTab === 'requests') requestsSearchHook.setSearchTerm(e.target.value);
                  else lateFilesSearchHook.setSearchTerm(e.target.value);
                }}
                className="pl-9 pr-3 py-1.5 bg-white border border-slate-200 rounded-xl text-xs outline-none text-slate-700 font-semibold focus:border-blue-500 w-48 sm:w-64 transition-all"
              />
            </div>
          </div>
          {activeTab === 'requests' && (
            <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm">
              {requestsSearchHook.filteredData.length === 0 ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs border-collapse">
                    ...
                    <tbody>
                      {requestsSearchHook.filteredData.map((req) => (
          ...
          {activeTab === 'delays' && (
            <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm">
              {lateFilesSearchHook.filteredData.length === 0 ? (
                <div className="py-12 text-center text-slate-400 text-xs font-semibold">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs border-collapse">
                    ...
                    <tbody>
                      {lateFilesSearchHook.filteredData.map((file) => (
