...
function DashboardOverview() {
  ...
  useEffect(() => {
    authFetch('http://localhost:8080/api/faculty/academic-calendar')
      ...
      .then(data => data && setAcademicCalendar(data))
      .catch(console.error);

    authFetch('http://localhost:8080/api/notifications')
      .then(res => res.ok && res.json())
      .then(data => {
        if(data) {
          const assignments = data.filter(n => n.type === 'AUDIT_ASSIGNMENT' || n.title?.includes('Audit Work Assigned'));
          setAssignedWork(assignments);
        }
      })
      .catch(console.error);
  }, []);
  ...
  return (
    <div>
      <PageHeader title="HOD Dashboard" subtitle="Assigned Work & Calendar" />
      
      <div style={{ marginBottom: '2rem' }}>
        <h3 style={{ fontSize: '1.1rem', fontWeight: 600, color: '#1e293b', marginBottom: '1rem' }}>
          Assigned Work from Invigilator
        </h3>
        <div style={{ background: '#fff', borderRadius: 10, padding: 16, boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
          {assignedWork.length > 0 ? (
            <Table
              columns={[
                { key: 'message', label: 'Assignment Details' },
                { key: 'createdAt', label: 'Assigned On', render: (v) => new Date(v).toLocaleDateString() }
              ]}
              data={assignedWork}
            />
          ) : (
            <p style={{ color: '#64748b', fontSize: '0.9rem', textAlign: 'center', padding: '20px 0' }}>
              No assigned audit work found.
            </p>
          )}
        </div>
      </div>
      ...
      {academicCalendar && (
        <div style={{ marginTop: '2rem' }}>
          <h3 style={{ fontSize: '1.25rem', fontWeight: 600, color: '#1e293b', marginBottom: '1rem' }}>
            Academic Calendar Matrix
          </h3>
          <VisualCalendarMatrix academicCalendar={academicCalendar} schedules={[]} />
        </div>
      )}
    </div>
  );
