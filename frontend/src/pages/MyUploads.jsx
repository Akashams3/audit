...
const MyUploads = () => {
  ...
  const fetchFiles = async () => {
    try {
      setLoading(true);
      const cRes = await authFetch(`http://localhost:8080/api/faculty/academic-files${buildYearParams()}`);
      const dRes = await authFetch(`http://localhost:8080/api/faculty/department-files${buildYearParams()}`);
      if (cRes.ok && dRes.ok) {
