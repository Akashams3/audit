  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      try {
        ...
        const res = await authFetch(`http://localhost:8080/api/faculty/required-files${buildYearParams(extras)}`);
        if (res.ok) setFiles(await res.json());
        const calRes = await authFetch(`http://localhost:8080/api/director/academic-calendar${buildYearParams()}`);
