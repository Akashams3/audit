...
const AcademicFileUpload = () => {
  ...
  const [studentYear, setStudentYear] = useState(selectedStudyYear || '1st Year');
  ...
  useEffect(() => {
    if (selectedStudyYear) {
      setStudentYear(selectedStudyYear);
    }
  }, [selectedStudyYear]);
  ...
  const fetchMyFiles = async () => {
    try {
      const res = await authFetch(`http://localhost:8080/api/faculty/academic-files${buildYearParams()}`);
      if (res.ok) {
  ...
  const fetchRequiredFiles = async () => {
    try {
      const res = await authFetch(`http://localhost:8080/api/faculty/required-files${buildYearParams({ year: studentYear })}`);
      if (res.ok) {
        const data = await res.json();
                      ...
                      <tr key={f.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors font-medium">
                        ...
                        <td className="py-3.5 px-2 text-right">
                          <div className="flex items-center justify-end space-x-1.5">
                            <a
                              href={`${API_BASE_URL}/api/files/download/academic/${f.id}?token=${localStorage.getItem('token')}`}
