...
const DirectorProgressPage = () => {
  ...
  const fetchProgress = async () => {
    try {
      setLoading(true);
      const res = await authFetch(`http://localhost:8080/api/director/faculty-invigilator-progress${buildYearParams()}`);
      if (res.ok) {
  ...
  const facultySearchHook = useFilterSortSearch(data.faculty, {
    searchKeys: ['name', 'code', 'department'],
    defaultSortKey: 'progress',
    defaultSortOrder: 'desc'
  });

  const invigilatorsSearchHook = useFilterSortSearch(data.invigilators, {
    searchKeys: ['name', 'department', 'departmentCode'],
    defaultSortKey: 'progress',
    defaultSortOrder: 'desc'
  });

  // Apply manual department filter on top of the hook if needed, or update hook
  const filteredFaculty = facultySearchHook.filteredData.filter(f => deptFilter === 'All' || f.department === deptFilter);
  const filteredInvigilators = invigilatorsSearchHook.filteredData.filter(i => deptFilter === 'All' || i.departmentCode === deptFilter);
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="relative">
              ...
              <input
                ...
                value={activeTab === 'faculty' ? facultySearchHook.searchTerm : invigilatorsSearchHook.searchTerm}
                onChange={e => {
                  facultySearchHook.setSearchTerm(e.target.value);
                  invigilatorsSearchHook.setSearchTerm(e.target.value);
                }}
