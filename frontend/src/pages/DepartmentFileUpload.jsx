...
const DepartmentFileUpload = () => {
  ...
  const fetchUploadStatus = async () => {
    try {
      const res = await authFetch(`http://localhost:8080/api/faculty/upload-status${buildYearParams()}`);
      if (res.ok) {
  ...
  const fetchMyFiles = async () => {
    try {
      const res = await authFetch(`http://localhost:8080/api/faculty/department-files${buildYearParams()}`);
      if (res.ok) {
  ...
  const fetchRequiredFiles = async () => {
    try {
      const res = await authFetch(`http://localhost:8080/api/faculty/required-files${buildYearParams()}`);
      if (res.ok) {
        const data = await res.json();
        setRequiredFiles(data.filter(r => r.fileCategory === 'DEPARTMENT'));
      }
    } catch (e) {
      console.error(e);
    }
  ...
  const handleUpload = async (e) => {
    e.preventDefault();
    if (!documentType) {
      setMsg({ text: 'Please select a document type.', type: 'error' });
      return;
    }
    ...
    try {
      ...
      if (selectedStudyYear && selectedStudyYear !== 'ALL') {
          formData.append('year', selectedStudyYear);
      }
      if (uploadStatus.schedulePhase) {
          formData.append('stage', uploadStatus.schedulePhase);
      }

      setProgress(50);
      const res = await authFetch('http://localhost:8080/api/faculty/department-files', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await authFetch(`http://localhost:8080/api/faculty/department-files/${id}`, {
        method: 'DELETE',
      });
      if (res.ok) {
  ...
  return (
    <div className="space-y-6 font-sans">
      <div>
        <h2 className="text-xl font-bold text-slate-800">Upload Department File</h2>
        <p className="text-xs text-slate-400 font-semibold mt-1">Dashboard &gt; Upload Department File</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm space-y-5">
            <div>
              <h3 className="text-sm font-bold text-slate-800">Upload Department File</h3>
              <p className="text-[10px] text-slate-400 font-semibold mt-0.5">Upload department related documents for audit.</p>
            </div>
                  ...
                  {submissionType === 'file' ? (
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        rows={8}
                        placeholder="Type or paste the text content for this department document..."
                        className="mt-1.5 block w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs text-slate-700 focus:bg-white focus:border-[#0A3D91] outline-none transition-all font-semibold"
                      />
                    </div>
                  )}
            ...
            <div className="bg-blue-50/60 border border-blue-100 rounded-xl p-4 space-y-2">
              ...
              <ul className="list-disc pl-5 text-[10px] text-blue-800 font-bold space-y-1">
                <li>Upload the complete department file as per the guidelines.</li>
          ...
          <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm">
            <h3 className="text-sm font-bold text-slate-800 border-b border-slate-50 pb-3 mb-4">My Uploaded Department Files</h3>
                  ...
                  <tbody>
                    {myFiles.map((f) => (
                      <tr key={f.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors font-medium">
                        ...
                        <td className="py-3.5 px-2 text-[#0A3D91] font-semibold truncate max-w-[150px]">{f.fileName}</td>
                        ...
                        <td className="py-3.5 px-2 text-right">
                          <div className="flex items-center justify-end space-x-1.5">
                            <a
                              href={`${API_BASE_URL}/api/files/download/department/${f.id}?token=${localStorage.getItem('token')}`}
        ...
        <div className="space-y-6">
          <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm text-center space-y-4">
            ...
            <div>
              <h4 className="font-bold text-slate-800 text-sm">About Department File</h4>
              <p className="text-[11px] text-slate-500 mt-2 leading-relaxed">
                Department files include faculty lists, infrastructural metrics, curricular activities, placement data, and related documents.
              </p>
            </div>
...
export default DepartmentFileUpload;
