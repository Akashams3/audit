...
const DirectorRequiredFilesPage = () => {
  ...
  const fetchFiles = async () => {
    ...
    try {
      const extras = {};
      if (selectedStageFilter) extras.stage = selectedStageFilter;
      if (selectedYearFilter) extras.year = selectedYearFilter;
      if (selectedSemesterFilter) extras.semester = selectedSemesterFilter;

      const res = await authFetch(`http://localhost:8080/api/director/required-files${buildYearParams(extras)}`);
  ...
  useEffect(() => {
    ...
    fetchRoles();
    fetchCalendar();
  }, [selectedStageFilter, selectedYearFilter, selectedSemesterFilter, selectedAcademicYear, selectedStudyYear]);
  ...
  const searchHook = useFilterSortSearch(files, {
    searchKeys: ['fileName', 'fileCategory', 'description']
  });

  const filtered = activeTab === 'ALL'
    ? searchHook.filteredData
    : activeTab === 'fpp'
    ? searchHook.filteredData.filter(isFppDocument)
    : activeTab === 'ACADEMIC'
    ? searchHook.filteredData.filter(f => f.fileCategory === 'ACADEMIC' && !isFppDocument(f))
    : searchHook.filteredData.filter(f => f.fileCategory === activeTab);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-100 pb-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-xs outline-none text-slate-700 font-semibold focus:bg-white w-full sm:w-48 transition-all"
            />
          </div>
        </div>
