...
const DirectorDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const params = buildYearParams();
      const [statsRes, sumRes, filesRes, feedbackRes, reqRes] = await Promise.allSettled([
        authFetch(`http://localhost:8080/api/director/dashboard${params}`),
        authFetch(`http://localhost:8080/api/director/department-summary${params}`),
        authFetch(`http://localhost:8080/api/director/files${params}`),
        authFetch(`http://localhost:8080/api/director/feedback${params}`),
        authFetch(`http://localhost:8080/api/director/required-files${params}`)
      ]);
      ...
      if (reqRes.status === 'fulfilled' && reqRes.value.ok) {
        try {
          ...
          setRequiredFiles(Array.isArray(reqData) ? reqData.filter(isFppDocument) : []);
        } catch (e) {}
      }
    } catch (err) {
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white border border-slate-100 rounded-2xl p-4 shadow-sm">
        ...
        <div className="flex items-center space-x-2 bg-blue-50 border border-blue-100 px-3 py-1.5 rounded-xl">
          <span className="text-xs font-bold text-blue-800">Year: {selectedStudyYear === 'ALL' ? 'All Years' : selectedStudyYear} | AY: {selectedAcademicYear === 'ALL' ? 'All AY' : selectedAcademicYear}</span>
        </div>
      </div>
                        ...
                        <div className="flex items-center justify-end space-x-2 text-slate-400">
                          <a
                            href={f.fileType === 'Academic File' || f.fileType === 'Course File'
                              ? `${API_BASE_URL}/api/files/view/academic/${f.id}?token=${localStorage.getItem('token')}`
                              : `${API_BASE_URL}/api/files/view/department/${f.id}?token=${localStorage.getItem('token')}`
                            }
                            target="_blank"
                            rel="noreferrer"
                          ...
                          <a
                            href={f.fileType === 'Academic File' || f.fileType === 'Course File'
                              ? `${API_BASE_URL}/api/files/download/academic/${f.id}?token=${localStorage.getItem('token')}`
                              : `${API_BASE_URL}/api/files/download/department/${f.id}?token=${localStorage.getItem('token')}`
                            }
