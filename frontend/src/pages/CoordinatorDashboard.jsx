...
function CoordinatorDashboard() {
   ...
   const { selectedAcademicYear, selectedStudyYear, yearParamsObj } = useYearFilteredApi();
  ...
  useEffect(() => {
    loadSectionData();
  }, [activeSection, filterYear, filterSem, selectedAcademicYear, selectedStudyYear]);
   ...
   const loadSectionData = async () => {
     ...
     try {
       switch (activeSection) {
         case "departments":
           try {
             const deptRes = await getDepartments(yearParamsObj);
         ...
         case "hods":
           try {
             const hodRes = await getHods(yearParamsObj);
         ...
         case "faculty":
           try {
             const facRes = await getFaculties(yearParamsObj);
         ...
         case "ccm":
           try {
             const ccmRes = await getCcmMembers(yearParamsObj);
         ...
         case "cocm":
           try {
             const cocmRes = await getCocmMembers(yearParamsObj);
         ...
         case "incharge":
           try {
             const inchargeRes = await getClassIncharges(yearParamsObj);
         ...
         case "mentor":
           try {
             const mentorRes = await getClassMentors(yearParamsObj);
         ...
         case "lessonplans":
           try {
             const lessonRes = await getLessonPlans({ academicYear: filterYear || undefined, ...yearParamsObj });
         ...
         case "eresources":
           try {
             const eResRes = await getEResources({ academicYear: filterYear || undefined, ...yearParamsObj });
         ...
         case "videoLectures":
           try {
             const videoRes = await getVideoLectures({ academicYear: filterYear || undefined, ...yearParamsObj });
         ...
         case "curriculum":
           try {
             const curricRes = await getCurriculums({ academicYear: filterYear || undefined, ...yearParamsObj });
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <div className="border-b border-gray-200 bg-white px-6 py-3 shadow-sm">
          <div className="flex justify-between items-center">
            ...
            <div className="text-sm font-semibold text-gray-600 bg-gray-100 px-3 py-1 rounded-md">
...
function DashboardOverview() {
  ...
  const { yearParamsObj, selectedAcademicYear, selectedStudyYear } = useYearFilteredApi();
  ...
  const fetchStats = async () => {
    try {
      const [dept, hod, fac, ccm, cocm, incharge, mentor, lesson, mat, eRes, video, time, curric] = await Promise.all([
        getDepartments(yearParamsObj).catch(() => ({ data: [] })),
        getHods(yearParamsObj).catch(() => ({ data: [] })),
        getFaculties(yearParamsObj).catch(() => ({ data: [] })),
        getCcmMembers(yearParamsObj).catch(() => ({ data: { data: [] } })),
        getCocmMembers(yearParamsObj).catch(() => ({ data: { data: [] } })),
        getClassIncharges(yearParamsObj).catch(() => ({ data: { data: [] } })),
        getClassMentors(yearParamsObj).catch(() => ({ data: { data: [] } })),
        getLessonPlans(yearParamsObj).catch(() => ({ data: [] })),
        getMaterials(yearParamsObj).catch(() => ({ data: [] })),
        getEResources(yearParamsObj).catch(() => ({ data: [] })),
        getVideoLectures(yearParamsObj).catch(() => ({ data: [] })),
        getTimetables(yearParamsObj).catch(() => ({ data: [] })),
        getCurriculums(yearParamsObj).catch(() => ({ data: [] }))
      ]);
      setStats({
