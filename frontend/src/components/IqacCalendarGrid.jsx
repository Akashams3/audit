...
const IqacCalendarGrid = ({ academicCalendar, schedules = [] }) => {
      ...
      monthList.forEach((m, idx) => {
        ...
        for (let dayNum = 1; dayNum <= daysInMonth; dayNum++) {
          ...
          if (events.length > 0) {
            ...
            switch (primaryCode) {
              case 2: fillRGB = [253, 224, 71]; textRGB = [113, 63, 18]; break; // Post CAT - Yellow
              ...
              case 6: fillRGB = [96, 165, 250]; textRGB = [255, 255, 255]; break;  // FPP Audit - Blue
              case 7: fillRGB = [168, 85, 247]; textRGB = [255, 255, 255]; break; // Exams - Purple
  ...
  const legends = [
    ...
    { code: 2, label: 'Post CAT exam Auditing', bg: 'bg-yellow-300 text-yellow-950', border: 'border-yellow-400' },
    ...
    { code: 6, label: 'Department file Auditing (FPP)', bg: 'bg-blue-400 text-white', border: 'border-blue-500' },
  ];
  ...
  // Build target ranges for FPP, Post CAT 1, 2, 3, End Sem
  const targetRanges = React.useMemo(() => {
    const ranges = { fpp: [], postCat1: [], postCat2: [], postCat3: [], endSem: [] };
    if (!academicCalendar) return ranges;

    const calc = (base, offset, count) => {
      if (!base) return [];
      const days = [];
      ...
      let toCollect = count;
      let collectCurrent = new Date(current);
      while (toCollect > 0) {
        if (collectCurrent.getDay() !== 0 && collectCurrent.getDay() !== 6) {
          days.push(collectCurrent.toISOString().split('T')[0]);
          toCollect--;
        }
        collectCurrent.setDate(collectCurrent.getDate() + 1);
      }
      return days;
    };

    ranges.fpp = calc(academicCalendar.reopeningDate, -10, 10);
    ranges.postCat1 = calc(academicCalendar.cat1EndDate, 1, 10);
    ranges.postCat2 = calc(academicCalendar.cat2EndDate, 1, 10);
    ranges.postCat3 = calc(academicCalendar.cat3EndDate, 1, 10);
    ranges.endSem = calc(academicCalendar.theoryExamDate || academicCalendar.lastWorkingDay, 1, 10);
  ...
  const getEventsForDate = (dateStr) => {
    ...
    schedules.forEach((s) => {
      if (s.auditDate === dateStr) {
        ...
        else if (s.academicPhase === 'POST_CAT_1' || s.academicPhase === 'POST_CAT_2' || s.academicPhase === 'END_SEM') legendCode = 2;
    ...
    if (academicCalendar) {
      ...
      if (academicCalendar.cat1Date === dateStr && !seenKeys.has('CAT I Start')) {
        matched.push({ title: 'CAT I Exam Starts', legendCode: 7, type: 'Exam', desc: 'Continuous Assessment Test I Start' });
      }
      if (academicCalendar.cat1EndDate === dateStr && !seenKeys.has('CAT I End')) {
        matched.push({ title: 'CAT I Exam Ends', legendCode: 7, type: 'Exam', desc: 'Continuous Assessment Test I End' });
      }
      if (academicCalendar.cat2Date === dateStr && !seenKeys.has('CAT II Start')) {
        matched.push({ title: 'CAT II Exam Starts', legendCode: 7, type: 'Exam', desc: 'Continuous Assessment Test II Start' });
      }
      if (academicCalendar.cat2EndDate === dateStr && !seenKeys.has('CAT II End')) {
        matched.push({ title: 'CAT II Exam Ends', legendCode: 7, type: 'Exam', desc: 'Continuous Assessment Test II End' });
      }
      if (academicCalendar.cat3Date === dateStr && !seenKeys.has('CAT III Start')) {
        matched.push({ title: 'CAT III Exam Starts', legendCode: 7, type: 'Exam', desc: 'Continuous Assessment Test III Start' });
      }
      if (academicCalendar.cat3EndDate === dateStr && !seenKeys.has('CAT III End')) {
        matched.push({ title: 'CAT III Exam Ends', legendCode: 7, type: 'Exam', desc: 'Continuous Assessment Test III End' });
      }
      ...
      if (academicCalendar.practicalExamDate === dateStr && !seenKeys.has('Practical')) {
        matched.push({ title: 'End Sem Practical Exam', legendCode: 7, type: 'Exam', desc: 'Practical Examinations' });
      }
      if (academicCalendar.theoryExamDate === dateStr && !seenKeys.has('Theory')) {
        matched.push({ title: 'End Sem Theory Exam', legendCode: 7, type: 'Exam', desc: 'Theory Examinations' });
      }

      let hasRealFpp = false;
      let hasRealPostCat1 = false;
      let hasRealPostCat2 = false;
      let hasRealPostCat3 = false;
      let hasRealEndSem = false;

      schedules.forEach((s) => {
        if (s.auditDate === dateStr) {
          if (s.academicPhase === 'FPP') hasRealFpp = true;
          if (s.academicPhase === 'POST_CAT_1') hasRealPostCat1 = true;
          if (s.academicPhase === 'POST_CAT_2') hasRealPostCat2 = true;
          if (s.academicPhase === 'POST_CAT_3') hasRealPostCat3 = true;
          if (s.academicPhase === 'END_SEM') hasRealEndSem = true;
        }
      });

      if (!hasRealFpp && targetRanges.fpp.includes(dateStr) && !seenKeys.has('FPP Target')) {
        matched.push({ title: 'FPP Academic Audit Target', legendCode: 6, type: 'Academic (FPP)', desc: 'Reserved for FPP Audit' });
        seenKeys.add('FPP Target');
      }
      if (!hasRealPostCat1 && targetRanges.postCat1.includes(dateStr) && !seenKeys.has('Post CAT 1 Target')) {
        matched.push({ title: 'POST CAT 1 Audit Target', legendCode: 2, type: 'Audit', desc: 'Reserved for Post CAT 1 Audit' });
        seenKeys.add('Post CAT 1 Target');
      }
      if (!hasRealPostCat2 && targetRanges.postCat2.includes(dateStr) && !seenKeys.has('Post CAT 2 Target')) {
        matched.push({ title: 'POST CAT 2 Audit Target', legendCode: 2, type: 'Audit', desc: 'Reserved for Post CAT 2 Audit' });
        seenKeys.add('Post CAT 2 Target');
      }
      if (!hasRealPostCat3 && targetRanges.postCat3.includes(dateStr) && !seenKeys.has('Post CAT 3 Target')) {
        matched.push({ title: 'POST CAT 3 Audit Target', legendCode: 2, type: 'Audit', desc: 'Reserved for Post CAT 3 Audit' });
        seenKeys.add('Post CAT 3 Target');
      }
      if (!hasRealEndSem && targetRanges.endSem.includes(dateStr) && !seenKeys.has('End Sem Target')) {
        matched.push({ title: 'END SEM Audit Target', legendCode: 2, type: 'Audit', desc: 'Reserved for End Sem Audit' });
        seenKeys.add('End Sem Target');
      }
    }
  ...
  const getStyleForDate = (events, dayOfWeek) => {
    ...
    const primaryCode = events[0].legendCode;
    switch (primaryCode) {
      case 2: return 'bg-yellow-300 text-yellow-950 font-bold shadow-xs';
      ...
      case 6: return 'bg-blue-400 text-white font-bold shadow-xs';
      case 7: return 'bg-purple-500 text-white font-bold shadow-xs';
