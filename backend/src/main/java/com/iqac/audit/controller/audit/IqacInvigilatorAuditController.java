...
public class IqacInvigilatorAuditController {
    ...
    public ResponseEntity<?> assignAuditWork(@RequestBody Map<String, Object> payload) {
        try {
            ...
            if (payload.containsKey("hodIds")) {
                List<?> hodIdsRaw = (List<?>) payload.get("hodIds");
                List<Long> hodIds = new ArrayList<>();
                for (Object id : hodIdsRaw) {
                    hodIds.add(Long.valueOf(id.toString()));
                }

                List<Hod> hods = hodRepository.findAllById(hodIds);
                String specificMessage = payload.containsKey("specificMessage") ? " (" + payload.get("specificMessage").toString() + ")" : "";
                for (Hod hod : hods) {
                    notificationService.createNotification(hod.getUser(),
                            "You have been assigned to prepare department files for the upcoming audit: " + schedule.getTitle() + specificMessage + ". Due date is " + schedule.getDueDate() + ".",
                            "AUDIT_ASSIGNMENT", "Audit Work Assigned");
                    
                    String html = "<html><body>" +
                            "<h3 style='color:#1A56DB;'>Audit Work Assigned</h3>" +
                            "<p>Dear " + hod.getName() + " (HOD),</p>" +
                            "<p>You have been assigned to prepare and submit your required department files for the upcoming audit:</p>" +
                            "<ul><li><strong>Audit Title:</strong> " + schedule.getTitle() + "</li>" +
                            "<li><strong>Audit Date:</strong> " + schedule.getAuditDate() + "</li>" +
                            "<li><strong>Due Date:</strong> " + schedule.getDueDate() + "</li></ul>" +
                            "<p>Please log in to the portal and upload your files before the deadline.</p>" +
                            "<p>Best regards,<br/>IQAC Invigilator</p></body></html>";
                    emailService.sendHtmlEmail(hod.getUser().getEmail(), "Audit Work Assigned: " + schedule.getTitle(), html);
                    totalAssigned++;
                }
            }

            return ResponseEntity.ok(Collections.singletonMap("message", "Work assigned successfully to " + totalAssigned + " members."));
        } catch (Exception e) {
    ...
    @GetMapping("/hod-status")
    public ResponseEntity<?> getHodStatus() {
        IqacInvigilator invigilator = getAuthenticatedInvigilator();
        String deptCode = invigilator.getDepartment().getCode();
        ...
        List<Map<String, Object>> hodList = new ArrayList<>();
        List<RequiredFile> requiredFiles = requiredFileRepository.findAll();

        for (Hod h : hods) {
            Map<String, Object> map = new HashMap<>();
            map.put("facultyId", h.getId()); // Use facultyId key for frontend compatibility
            map.put("name", h.getName());
            map.put("facultyCode", "HOD");
            map.put("email", h.getUser().getEmail());
            map.put("designations", "Head of Department");
            map.put("facultyRoles", "HOD");

            List<DepartmentFile> deptFiles = departmentFileRepository.findByDepartment(deptCode);
            ...
            map.put("academiaFilesSubmitted", 0);
            map.put("academiaFilesTotal", 0);
            map.put("courseFilesSubmitted", 0);
            map.put("courseFilesTotal", 0);
            map.put("departmentFilesSubmitted", deptSubmitted);
            map.put("departmentFilesTotal", deptExpected);
            map.put("totalFilesCount", deptSubmitted);
